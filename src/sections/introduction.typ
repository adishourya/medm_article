...
In addition to these advancements, there has been significant progress in improving model performance, consistency, and accessibility. This has enabled individual researchers to perform not just inference but also to fine-tune models for cheap.These, along with other advancements, continue to bring attention back to  Medical LLMs. However, much of this interest is possibly inflated. As the instruct models are often trained on high-quality organic datasets with vast data volumes, which sometimes is even a good chunk of the internet, while being diverse,they lack sufficient medical content. Although post-training innovations have enabled these models to perform well on a range of downstream tasks, they still struggle when applied to the specialized and complex demands of medical AI.
