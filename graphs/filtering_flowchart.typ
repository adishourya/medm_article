...
#let node_values = (
  ...
  llm : [#emoji.robot LLM],
  literature : [Literature Based\ Questions],
  related_image : [Image Related\ Questions],
...
#diagram(
   ...
   // nodes
	...
	blob(nodes.filter0,node_values.pathology, tint:my_colors.accent10),
	blob(nodes.llm,node_values.llm, tint: white),
	blob(nodes.bucket0,node_values.literature, tint: my_colors.accent11.darken(10%)),
	blob(nodes.bucket1,node_values.related_image,tint:my_colors.accent11),
	blob(nodes.filter,node_values.filter,tint:my_colors.accent10,shape:hexagon),
   ...
   edge(nodes.filter0,nodes.llm,stroke:(thickness:1.5pt,paint:my_colors.accent8),"-|>"),
   edge((0,2),(-0.2,2),"-"),
